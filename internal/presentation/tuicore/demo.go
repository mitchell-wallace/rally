...
func DemoScript() []DemoStep {
	...
	return []DemoStep{
		...
		runHeaderStep(150*time.Millisecond, 0, "opencode", "zai-coding-plan/glm-5.2", "Baseline check and capture agent's exported surface", 1, start),
		...
		runHeaderStep(250*time.Millisecond, 1, "codex", "gpt-5.5", "Phase 1: introduce internal/harnessapi contract and re-point all consumers", 2, start.Add(5*time.Minute)),
		...
		runHeaderStep(250*time.Millisecond, 2, "codex", "gpt-5.4", "Verify Phase 1 contract seam", 3, start.Add(22*time.Minute)),
		...
		runHeaderStep(350*time.Millisecond, 3, "opencode", "glm-5.2", "Phase 2: extract internal/harness/process support package", 4, start.Add(56*time.Minute)),
		...
		{250 * time.Millisecond, runtimeevent.RelaySummaryReady{TotalRuns: 4, Passed: 3, Failed: 1, TotalDuration: time.Hour + 35*time.Second}},
		{100 * time.Millisecond, runtimeevent.RelayCompleted{RelayID: 42, TotalRuns: 4, Passed: 3, Failed: 1, TotalDuration: time.Hour + 35*time.Second}},
	}
}
...
func runHeaderStep(delay time.Duration, runIndex int, agent, model, lapTitle string, lapsStarted int, start time.Time) DemoStep {
	return DemoStep{Delay: delay, Event: runtimeevent.RunHeaderReady{
		RunIndex:     runIndex,
		TotalRuns:    14,
		AgentName:    agent,
		Attempt:      1,
