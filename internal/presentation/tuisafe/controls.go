...
func (c *controls) Start(ctx context.Context) (<-chan runtimeevent.Press, error) {
	...
	defer c.mu.Unlock()
	if c.active {
		return nil, errors.New("tuisafe controls already started")
	}
	...
	// The watcher must only stop the session it belongs to: a cancelled
	// earlier context racing a later Start must not close the newer channel.
	go func() {
		<-ctx.Done()
		c.mu.Lock()
...
// stopSessionLocked closes the session owning ch; it is a no-op when ch is not
// the current session (already stopped, or superseded by a later Start).
