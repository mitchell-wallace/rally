...
- After every phase, add at least one `VERIFY` lap for that phase. Add a second targeted review lap for high-risk changes.
- After the final phase, add one additional `VERIFY` lap for the full OpenSpec change or full non-OpenSpec outcome.
