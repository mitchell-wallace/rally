...
| Harness | Slug | Notes |
|---|---|---|
| `ag`/`agy` (antigravity) | `Gemini 3.5 Flash (High)` | **agy is UNAUTHENTICATED in this container** (re-probed 2026-07-10; OAuth is interactive-only — ask the user to run `agy` auth during an availability window). `TestRealBackend_AntigravityRelay` fails with `all agents unavailable` after ~30s — environmental, not a rally bug. Slug itself is correct; `agy` 1.0.15 has no CLI model flag, so Rally sets `~/.gemini/antigravity-cli/settings.json` for the run and restores it. |
...
| `cx` (codex) | `gpt-5.6-luna` | Smallest/fastest of the GPT-5.6 family (verified live 2026-07-10); prefer for smoke tests. Family: `gpt-5.6-sol` (frontier), `gpt-5.6-terra` (mid), `gpt-5.6-luna`. `gpt-5.4-mini` and the 5.3/5.4/5.5 slugs remain available. User config shorthands: cx:sol/terra/luna. |
