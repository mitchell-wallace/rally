...
- `[harness.<name>]` — per-harness configuration: `models` sub-table for named models; `command` (array of strings) and `output_strategy` / `output_lines` for user-defined harnesses (see `harness-models` capability)
