...
- `harness` and `model` surface as separate fields throughout the executor layer.
...
- A `[harness.<name>]` entry that declares a `command` field registers a new harness:
  ...
  glm     = "glm-5.1"
  ```
- The `command` array is templated at run time: `$MODEL` is replaced with the resolved model string; `$PROMPT` (if present) is replaced with the prompt body. If `$PROMPT` is absent from the command, the prompt is piped on stdin.
- `output_strategy = "tail"` is the only strategy in v0.5.0; `output_lines` defaults to 40 if omitted. The tail parser captures the last N lines of the harness's stdout/stderr and surfaces them as the run's output.
- Built-in harnesses (`cc`/`cx`/`ge`/`op`) reject `command` and `output_strategy` fields at config load — their behaviour stays hard-coded. They may still declare `[harness.X.models]`.
...
- `[harness.*]`: per-harness configuration (named models, optionally `command`/output strategy for user harnesses).
...
- `harness-models`: per-harness named model bindings under `[harness.<name>.models]`, plus user-defined harnesses via `[harness.<name>]` with a templated `command` and a tail-N output parser.
...
- `relay-runner`: Mix parsing extended to resolve `harness:model-name` entries via the config layer; defaults sourced from `[defaults]`; fallback prompt content sourced from `[fallback].instructions_file` in no-backend mode; user-defined harnesses dispatched through the templated-command + tail-parser path.

## Impact

- Extends `internal/config/config_v2.go` schema (or splits into `v3.go` if the diff warrants).
- Adds a generic harness executor that runs a templated command and applies the tail-N output parser.
...
- Note: the legacy env-style `.rally/config` file has no loader in tree (already removed); no work needed there.
