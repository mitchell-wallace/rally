...
This would enable testing the full executor → subprocess → hook pipeline end-to-end, including hook registration, CLI flag construction, and output parsing — things that FixtureExecutor bypasses by operating above the Executor interface.
...
**Why not now**: FixtureExecutor covers the critical testing needs for v0.2.0. Mock CLI binaries add build complexity (separate binaries, PATH manipulation in tests) and are most valuable once the hook integration is stable and needs regression testing.
