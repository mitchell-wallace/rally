...
- [ ] 1.2 Capture the exported-identifier set of `internal/relay` as a checksum (`go doc ./internal/relay` or a grep of exported decls), to compare against the post-split `internal/relay` + `internal/relay/runner` sets. Note `CancellationSource` and its consts are exported and will move to `runner`; no new exported route-label constants/helpers should appear.
- [ ] 1.3 Capture a test/benchmark inventory, not just a count: pre-change `(file, Test*/Benchmark* name)` for all `internal/relay/*_test.go` functions. Keep a migration checklist so every pre-change function appears exactly once after the split, even when its package/file changes.
- [ ] 1.4 Capture comparable coverage baseline: `go test -coverprofile=/tmp/rally-relay-cover-before.out ./internal/relay/...`, then record the `go tool cover -func=/tmp/rally-relay-cover-before.out` total. Coverage for the post-split `./internal/relay/...` packages must not drop.
