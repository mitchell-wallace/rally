...
- [ ] 2.1 Create `tools/archguard` as `package main` in the existing module, stdlib-only (`go/parser`, `go/token`, `go/ast`, `os`, `path/filepath`, `bufio`, `strings`). Confirm `go mod tidy` produces no diff (no new dependency).
...
- [ ] 4.1 Encode the flagship/composition-root edges: `internal/relay` ↛ `internal/relay/runner`; `relay`/`relay/runner` ↛ `config`/`cli`; `internal/release` ↛ `internal/app`; `internal/app` ↛ {`cli`, `user_prompt`, `laps`}; no `internal/*` imports `internal/cli`.
- [ ] 4.2 Encode the per-package internal allow-lists from `design.md` Decision 4 (lower packages tight; `cli`/`cmd/rally` are the broad composition layers). Confirm the current tree passes.
- [ ] 4.3 Encode dependency confinement (Decision 5): New Relic → `telemetry`; `go-toml` → `config`; Cobra → {`cmd/rally`, `cli`, `progress`}; huh → {`cli`, `user_prompt`}; lipgloss → {`style`, `cli`}. Apply to production and test files alike.
...
- [ ] 5.1 Add a `just arch-check` recipe (`go run ./tools/archguard`) and make `just check` depend on `arch-check` (after `vet`/format). `just check` stays green.
