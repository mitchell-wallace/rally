...
- **WHEN** `[routes]` declares a `MARKETING` role but no current lap carries `assignee: MARKETING`
- **THEN** `rally routes check` SHALL emit an info-level message indicating the route is declared but not currently referenced by any lap; this SHALL NOT cause non-zero exit

#### Scenario: Missing default with non-default routes warned
- **WHEN** `[routes]` declares non-default routes but no `default` route
- **THEN** `rally routes check` SHALL emit a warning indicating that laps with no matching role would cause exit at run-time; this SHALL NOT by itself cause non-zero exit (it's a config style warning, not an error)
...
- **`default` route invalid or missing AND `[routes]` is otherwise non-empty** → same warn-and-prompt as above; if no laps exist in queue, warn-and-exit instead of prompting
...
#### Scenario: No default and no laps
- **WHEN** `rally relay` starts with `[routes]` declaring only non-default roles (no `default`) and the lap queue is empty
