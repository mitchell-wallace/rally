...
`.rally/` mixes human-edited config (`config.toml`, `agents/`, `README.md`), a user-facing run digest (`progress.yaml`), machine-churned JSONL records (`tries.jsonl` ~1.8MB, `relays.jsonl`, `agent_status.jsonl`, `messages.jsonl`), audit/state files (`hook-audit.jsonl`, `run-state.json`), a large transient prompt (`current_task.md` ~120KB), and dead legacy dirs (`batches/`, `relays/`). Verbose per-try/per-relay logs already live elsewhere — in `dataDir` (`~/.local/share/rally/`, configurable). Paths are built ad-hoc with `filepath.Join(..., ".rally", ...)` across `internal/store`, `internal/progress`, `internal/cli`, and `internal/relay`.
...
- **Migration data loss / partial moves** → Idempotent, move-don't-copy with existence checks; never overwrite an existing `state/` target; `progress.yaml` is left untouched (not converted); covered by tests on a fixture `.rally/`.
