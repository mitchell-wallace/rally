...
- [x] 4.1 Document the commit conventions (setup, lap-boundary, folded state, leftover-work guidance) in `AGENTS.md`/`README.md`
- [x] 4.2 Confirm sequencing: this change lands after `tidy-rally-runtime-data-storage` so the gitignore/state layout it commits against exists. Verify the tracked-file set in the working tree matches the intended set — `.rally/.gitignore`, `.rally/README.md`, `.rally/config.toml`, `.rally/agents/` (`*.md`), and `.rally/summary.jsonl` (the only churning tracked data file) — with no missing, extra, or legacy state files. (`.rally/agent_status.jsonl`, a tracked-churn artifact from an older rally version, was untracked during batch prep; confirm it stays gitignored.)
...
- [x] 4.5 Tests: commit messages with special characters (quotes, newlines) in lap descriptions are handled safely
- [x] 4.6 Tests: `git` unavailable or repo-corrupted scenarios log a warning and skip commits gracefully
- [x] 4.7 Bump `internal/buildinfo/VERSION` (patch bump per release process) as part of the change
