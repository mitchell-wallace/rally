...
- [x] 2.2 In `internal/relay/runner.go`, classify a try as incomplete when it produces file changes (dirty working tree) but the agent did not finalize the lap (no `laps done`/`laps handoff` detected)
