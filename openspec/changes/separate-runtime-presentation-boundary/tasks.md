...
- [ ] 1.1 Confirm the tree is green: `go build ./...`, `go vet ./...`, `gofmt -l .` (empty), `go test -count=1 ./...`, `go run ./tools/archguard --ci` (exit 0). If red, STOP.
- [ ] 1.2 Regenerate the presentation wiring inventory (the design's Context section, baselined at `f55712c`): runner `internal/style`/`internal/keyboard` import sites, direct `os.Stdout`/`os.Stderr`/`os.Stdin` uses (`run_one.go`, `relay_steps.go`, `task.go`, `terminal.go`, `handoff_only.go`), keyboard construction sites, monitor lifecycle calls, and the print-site output inventory (design Decision 3 table). This inventory is the emit-site checklist for phases 3–5.
- [ ] 1.3 Coordination check: this change's `composition-root-structure` delta text builds on `modularize-harness-adapters`' un-archived delta for the same requirement. Confirm #4 has been archived/synced first (or reconcile the requirement text at archive time).
...
- [ ] 4.3 Cut over print-site-by-print-site: delete each runner inline print once the sink renders its event; preserve exact call order relative to `mon.Stop()`/monitor renders (footer-parking contract). Finish with zero direct operator-facing `os.Stdout`/`os.Stderr` writes in runner production code and the `internal/style` import removed from all runner files.
...
- [ ] 5.1 Implement `terminal.Controls`: wraps `keyboard.NewKeyboard(os.Stdin, os.Stdout)` with raw-mode session per `Start`/`Stop`, translates `keyboard.Press` → `runtimeevent.Press`, `WaitResume` performs today's leave-raw-then-read-Enter sequence. Adapter test for repeated Start/Stop cycles (no leaked readers — mirror `keyboard_test.go`'s pipe-driven approach).
