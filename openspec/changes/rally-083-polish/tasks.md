...
- [ ] 3.1 In `parseOpenCodeOutput` (`internal/agent/opencode.go`), stop emitting raw stdout as `Summary`; on empty/unparseable output return `Completed=false` with a short bounded indicator
- [ ] 3.2 Apply the corrected event extraction from task 1.3: ordered `text` / `part.text`, tool counting, `step_finish` completion detection, process exit status, and top-level `error` events with bounded `error.data.message` / `ref`
...
- [ ] 7.1 Rename `internal/prompt` → `internal/user_prompt` and update all import sites
- [ ] 7.2 Create `internal/agent_prompt` with `general/` and `roles/` subfolders and a `go:embed` of the `.md` tree
- [ ] 7.3 Add `general/finalize.md` (commit + `laps done`/`laps handoff` + up-front `laps wrapup`) and `general/headless.md` (non-interactive; take intent from referenced planning docs)
- [ ] 7.4 Migrate `.rally/agents/{junior,senior,ui,verify}.md` content into `roles/*.md` as embedded defaults, stripping the shared finalize block now provided by `general/`
- [ ] 7.5 Document and implement prompt composition as a template: shared `general/` snippets are always included, the role slot is filled by on-disk `.rally/agents/<role>.md` when present or embedded `roles/<role>.md` otherwise, then existing task context is appended; preserve explicit `RunOptions.Prompt` override semantics and existing prompt sections for project instructions, role/persona guidance, task name/requirements, inbox/relay messages, previous summary, and recent try context
...
- [ ] 7.7 Add a note to `AGENTS.md` that prompt package naming reflects who is being prompted (`user_prompt` vs `agent_prompt`)
