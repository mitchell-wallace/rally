...
### Requirement: Route selection by lap `assignee`
...
2. The `[routes]` entry whose key matches the lap's `assignee` field case-insensitively
3. The `default` route, if step 2 produced no match (or the lap has no `assignee`)
...
- **WHEN** the active lap has `assignee: SENIOR` and `[routes].SENIOR` is defined
...
- **WHEN** the active lap has `assignee: ROLEX` (no matching `[routes]` key) and `[routes].default` is defined
...
- **WHEN** the active lap has `assignee: ROLEX` and neither `[routes].ROLEX` nor `[routes].default` is defined
