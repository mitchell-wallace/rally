...
- [x] 6.1 Add `github.com/getsentry/sentry-go` dependency
- [x] 6.2 Define a `telemetry.Sink` interface with a default no-op implementation
- [x] 6.3 Implement the Sentry sink: activate only with a configured DSN; honor `[telemetry] sentry_dsn`, `SENTRY_DSN` (overrides config), and `RALLY_TELEMETRY=0` kill switch
...
- [x] 6.8 Init the sink once in `cmd/rally/main.go` and `defer sentry.Flush(2s)` before exit (no-op/bounded when disabled or offline)
