...
- [ ] 1.1 Grep both rally and laps codebases to confirm no code generates a `.laps/.gitignore`; add a regression test asserting `laps.InstallHooks` never writes one (the stray `.laps/.gitignore` was already removed and `laps.json` already tracked as of commit `33d288e`)
...
- [ ] 5.3 Remove legacy `.rally/batches/`, top-level `.rally/relays/` log dir, and `.rally/config.toml.bak` (if present — these are legacy artifacts that may not exist in all repos)
...
- [ ] 6.5 Emit per-try structured logs at `store.AppendTry` (`runner.go:1045`) and model relay/run/try as a trace+span hierarchy; tag every event with `relay_id`/`run_id`/`try_id`/`role`/`runner`/`repo`/`lap_ids`
...
- [ ] 7.1 Update `install.sh` to fetch and install a compatible `laps` binary next to `rally` (from https://github.com/mitchell-wallace/rally)
